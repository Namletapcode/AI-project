...
TRAINING_STATE_PATH = f'{FOLDER_PATH}/batch_interval.pkl'

class VisionNumpyBatchIntervalAgent(BaseAgent):
    ...
    def train(self, render: bool = True, show_graph: bool = True):
        ...
        while True:
            ...
            while not game_over and episode_score < self.stop_on_score:
                ...
                if not np.any(current_state) and self.game.score < MAX_SKIP_FRAME:
                ...
                if self.step_count % TRAIN_INTERVAL == 0 and len(self.memory) >= MIN_MEMORY:
                    ...
                    states = np.zeros((BATCH_SIZE, (IMG_SIZE ** 2) * 2), dtype=np.float64)
            ...
            if episode_score >= self.stop_on_score:
                print(f"Game {self.number_of_games} finished after {self.episode} episodes")
                break
            if episode_score > self.best_score:
                log_message = f"{datetime.now().strftime(DATE_FORMAT)} Episode {self.episode}: New best score: {episode_score:0.1f} ({(episode_score-self.best_score)/self.best_score*100:+.1f}%)"
                print(log_message)
                with open(LOG_PATH, 'a') as log_file:
                    log_file.write(log_message + '\n')
                self.best_score = episode_score
                self.model.save(self.episode, True)
            elif self.episode % 300 == 0:
                self.model.save(self.episode, False)
            ...
            if self.number_of_games % 5 == 0:
                plot_training_progress(self.scores_per_episode, title='Vision_numpy Training', show_graph=show_graph, save_dir=GRAPH_PATH)
...
if __name__ == '__main__':
    agent = VisionNumpyBatchIntervalAgent(Game())
    mode = "train"
