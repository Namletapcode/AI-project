...
from utils.bot_helper import old_plot_training_progress, get_screen_shot_gray_scale, show_numpy_to_image
...
class VisionNumpyLongShortAgent(BaseAgent):
    ...
    def __init__(self, game: Game, load_saved_model: bool = False):
        ...
        # testing: critical memory
        self.critical_memory = deque(maxlen=MAX_CRI_MEM_SIZE)
    ...
    def train_long_memory(self):
        ...
        # testing: critical memory
        if len(self.critical_memory) <= MAX_CRI_MEM_SIZE:
            # if have not saved over 1000 states yet
            mini_sample = self.critical_memory
        else:
            # else pick random 1000 states to re-train
            mini_sample = random.sample(self.critical_memory, MAX_CRI_MEM_SIZE)
        for current_state, action, reward, next_state, game_over in mini_sample:
            self.train_short_memory(current_state, action, reward, next_state, game_over)
    ...
    def train(self, render: bool = True):
        ...
        while True:
            ...
            if game_over:
                ...
                # testing: critical memory
                self.critical_memory.append((current_state, action, reward, next_state, game_over))
                ...
                scores.append(score)
                old_plot_training_progress(scores, mean_scores)
